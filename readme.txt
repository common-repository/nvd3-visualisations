...
1. Supports open data input from any public site & let you copy data in (auto)magically.
...
1. Chart's picker gives your users many views of the same data set easily.
...
1. Copy&paste data table from OpenOffice Calc to the automatically updated chart.
1. Tag any pieces of page/post and build a new chart for numbers.
1. Accurate layout of each chart based on any HTML container tag.
1. Many charts on the same page/post of WordPress from same shortcode.
...
1. Chart calculator let users to modify chart fast by any basic math formulas.
1. Localized presentation formats for currency, date, time, etc.
1. Optional export of data / vector graphics of chart (SVG).
1. Works best with Opera, Chrome, and Firefox browsers. 
1. WordPress multisite & SSL compatible plugin.
1. Same open source code for WordPress & Drupal 7 societies: makes life easier for both group of developers.
.

**Data Input**

1. Fast & short data input from easy to remember options (values, labels, and series) of shortcode.
1. Direct data input from some html tags of document (eq page/post).
1. Data's import from static JSON/XML/CSV/TSV file formats.
1. Standard mySQL input from its table of database.
1. Dynamic JSON inputs from your own JavaScript functions.
1. Open data's URL path.
.
...
1. Create a new page (Pages / Add New) for charts gallery by calling [Charts_Gallery] shortcode.
1. Clear all sidebars of page (eq template "No Sidebars" setting) if your theme allows it.
1. Open+bookmark this page for future and start to clone your charts & edit them (there is a link for that on gallery).
1. Clone & enjoy your brand-new charts on WordPress !
...
1. Create a new page (Pages / Add New) for charts gallery by calling [Charts_Gallery] shortcode.
1. Clear all sidebars of page (eq template "No Sidebars" setting) if your theme allows it.
1. View+bookmark this page for future and start to clone your charts & edit them (there is a link for that on gallery).
1. Clone & enjoy your brand-new charts on WordPress !
1. More tips: go to nvd3 plugin's edit (Plugins / Installed plugins / NVD3 Visualisations / edit) and start to read from read_me_1st.txt file.
